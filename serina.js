...
queryURL = "https://api.openweathermap.org/data/2.5/forecast?q=" + location + "&units=imperial&appid=1dd029263e9a36200e5256b3f8ab4a28";
$.ajax({
    url: queryURL,
    method: "GET"
     }).then(function (response) {
     ...
     for(var i = 0; i < forecast_array.length; i++){
      ...
      console.log('Forecast ' + forecast_array[i].main.temp);
     }
 //Hardcoded the weather to show the first index found in the Main array seems to be the only way it will work
        $("#modal_p2").text("Temp: " + response.main.temp + "°F");
      $("#modal_p2").text("Forecast: " + response.list[i].main.temp);
